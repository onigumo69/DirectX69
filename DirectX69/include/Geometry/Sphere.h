...
{
public:
	static IndexedTriangleList MakeTesselated(meta::VertexLayout layout,
		int latDiv, int longDiv)
	{
		...
		meta::VertexBuffer vb{ std::move(layout) };
	...
	static IndexedTriangleList Make(std::optional<meta::VertexLayout> layout = std::nullopt)
	{
		using Element = meta::VertexLayout::ElementType;
		if (!layout)
		{
			layout = meta::VertexLayout{}.Append(Element::Position3D);
		}
